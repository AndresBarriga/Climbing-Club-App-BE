...
const showOtherProfileRouter = express.Router();



showOtherProfileRouter.get('/onlyProfile', authenticateToken, (req, res) => {
  // Extract the user_id from the request
  const user_id = req.query.userId;
 
  
  // SQL query to get the user details from the database
  const sqlUser = `SELECT user_id, name, last_name, profile_picture FROM users WHERE user_id = \$1`;
...
showOtherProfileRouter.get('/profileAndPreferences', authenticateToken, (req, res) => {
    // Extract the user_id from the request
    const user_id = req.query.userId;
...
export default showOtherProfileRouter;